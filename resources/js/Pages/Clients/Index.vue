...
const currentFranchiseeLabel = computed(() => {
  ...
  const franchisee = props.franchisees.find(f => String(f.id) === selectedFranchisee.value);
  return franchisee ? franchisee.name : 'Filtrar por franqueado...';
});


watch([search, selectedFranchisee], debounce(([searchValue, franchiseeIdValue]) => {
  const finalFranchiseeId = franchiseeIdValue === null ? '' : franchiseeIdValue;
  router.get(route('clients.index'), { search: searchValue, franchise_id: finalFranchiseeId }, {
    preserveState: true,
    replace: true,
    preserveScroll: true,
  });
  ...
  <AuthenticatedLayout>
    ...
    <div class="mx-auto w-full">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
        <div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto"> <div class="relative w-full sm:max-w-xs">
            <Input
              type="text"
              v-model="search"
          ...
          <div v-if="user.role === 'admin'" class="w-full sm:w-auto">
            <Popover v-model:open="openFranchiseeCombobox">
              <PopoverTrigger as-child>
                ...
                >
                  {{ selectedFranchisee ? currentFranchiseeLabel : "Filtrar por franqueado..." }}
                  <ChevronsUpDown class="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent class="w-full sm:w-[280px] p-0">
                <Command>
                  <CommandInput class="h-4 my-2" placeholder="Buscar franqueado..." />
                  <CommandList>
                    ...
                    <CommandGroup>
                      ...
                      >
                        <Check
                          :class="cn(
                            'mr-2 h-4 w-4',
                            selectedFranchisee === null ? 'opacity-100' : 'opacity-0',
                          )"
                        />
                      ...
                      >
                        <Check
                          :class="cn(
                            'mr-2 h-4 w-4',
                            selectedFranchisee === String(franchisee.id) ? 'opacity-100' : 'opacity-0',
                          )"
                        />
                        {{ franchisee.name }}
                      </CommandItem>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead class="px-4 py-3 text-gray-800 dark:text-gray-200">Franqueado Associado</TableHead>
        ...
        <template v-if="clients.data.length > 0">
          <Card v-for="client in clients.data" :key="`mobile-${client.id}`" class="bg-gray-100 dark:bg-gray-800 shadow-sm">
            ...
            <CardContent class="text-sm text-gray-700 dark:text-gray-300 space-y-1 pt-2 pb-4 px-6">
              ...
              <p v-if="client.franchisee_name"><strong class="font-medium text-gray-800 dark:text-gray-200">Franqueado:</strong> {{ client.franchisee_name }}</p>
            </CardContent>
